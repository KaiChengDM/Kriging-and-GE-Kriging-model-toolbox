function [x_design objective  GEKriging_obj GEKriging_const EI ] = GEKriging_optimization1(inputpar,hyperpar,subopt) 

% Adaptive  Sliced GE-Kriging  for global optimization
...
gaoptions  =  optimoptions('ga','UseParallel', true, 'UseVectorized', false, 'Display','off',...
          'FunctionTolerance',1e-7, 'PopulationSize', 300, 'MaxGenerations', 3000); % GA options
opts.LBounds = lb; opts.UBounds = ub; % CMAES options
iter      = 0; 
...
while (1)
   ...
   GEKriging_obj = SGEKriging_fit_2(inputpar,hyperpar);         %  Sliced GE-Kriging model for objective function
   ...
   for k = 1 : num_const
     ...
     GEKriging_const{k} = SGEKriging_fit_2(inputpar,hyperpar);  % Sliced GE-Kriging model for constraint function
...
%   [obj_mean,obj_varaince] = GEKriging_predictor(x_next,GEKriging_obj);
%   z(iter) = (min_current-obj_mean)./sqrt(obj_varaince);
%   sigma(iter) = sqrt(obj_varaince);
