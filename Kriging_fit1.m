function model = Kriging_fit1(inputpar,hyperpar)   

% Training a Kriging model with first order polynomial mean
...
if  ~isinf(f)
  % Iterate
  p  =  length(t);
  if  p <=  4,  kmax  =  4; else,  kmax  =  min(p,5); end
  for  k  =  1 : kmax
    th  =  t;
