...
% g = @(x)exp(sum((x.*(1:n))')/n^2+log(sum(((x.^2+x.^4).*(1:n))'))/n)';
% 
% for i = 1 : n
%   Pd{i} = @(x)exp(sum((x.*(1:n))')/n^2)*i/n^2+(i*(2.*x(:,i)+4.*x(:,i).^3))./(sum(((x.^2+x.^4).*(1:n))')')/n;
% end
...
for k = 1 : 5
 ...
 lb = -1.*ones(1,n);  ub = 1.*ones(1,n); N = Samplesize(k); N1 = 1000; 
