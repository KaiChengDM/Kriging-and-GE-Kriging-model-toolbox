...
g = @(x) 10.*x(:,1).^2 + 8.*x(:,2).^2 -100.*cos(pi*x(:,1)./1).*cos(3*pi*x(:,2)./sqrt(2));

Pd{1} = @(x)20.*x(:,1) +pi*100.*sin(pi*x(:,1)./1).*cos(3*pi*x(:,2)./sqrt(2));
Pd{2} = @(x)16.*x(:,2) +3*pi*100.*cos(2*pi*x(:,1)./1).*sin(3*pi*x(:,2)./sqrt(2))./sqrt(2);

% g = @(x)(10*n+(x(:,1).^2-10.*cos(2*pi*x(:,1)))+(x(:,2).^2-10.*cos(3*pi*x(:,2)))); % RASTRIGIN FUNCTION
% 
% Pd{1} = @(x)(2.*x(:,1)+20*pi.*sin(2*pi*x(:,1)));
% Pd{2} = @(x)(2.*x(:,2)+30*pi.*sin(3*pi*x(:,2)));

 for kk = 1:1
 
 sig = ones(1,n); mu = zeros(1,n);
 lb = -5.12.*ones(1,n);  ub = 5.12.*ones(1,n); N = 10;  N1 = 1000;
