function model = SGEKriging_fit_2(inputpar,hyperpar)   

% Training a 2 appendant sliced gradient-enhanced Kriging model
...
%% Split sample set into m parts
 ...
 for i = 1 : snum - 1
     ...
     ind2{i} = sort([ind{i};ind{i+1}]);
     ind1{i} = sort(ind{i+1});

     u2{i} = u(ind2{i},:); n2 = length(ind2{i});
     u1{i} = u(ind1{i},:); n1 = length(ind1{i});
     
     y2{i} = reshape(yt(ind2{i},1:dim+1),n2*(dim+1),1); 
     y1{i} = reshape(yt(ind1{i},1:dim+1),n1*(dim+1),1);
    
     f2{i} = [ones(n2,1); zeros(n2*dim,1)];  
     f1{i} = [ones(n1,1); zeros(n1*dim,1)];  
...
function Likelihood = GEKriging_likelihood(beta) 
 ...
 for i = 1 : snum-2
      ...
      sub_mat1 = feval(corr_fun,u1{i},theta,dim,'on');

      [upper_mat1{i} rd1] = chol(sub_mat1); 

      logdet1(i) = 2*sum(log(diag(upper_mat1{i})));  
 ...
 beta = (sum(nom2)-sum(nom1))/(sum(denom2)-sum(denom1));
   
 for i = 1 : snum-2
     sig2(i) = sum((upper_mat2{i}'\(y2{i}-beta*f2{i})).^2); 
     sig1(i) = sum((upper_mat1{i}'\(y1{i}-beta*f1{i})).^2); 
 end

 sig2(snum-1) = sum((upper_mat2{snum-1}'\(y2{snum-1}-beta*f2{snum-1})).^2); 

 sigma2 = (sum(sig2)-sum(sig1))/mn; 
  
% Likelihood = sigma2*prod(det2)/prod(det1);
  Likelihood = mn*log(sigma2)+sum(logdet2)-sum(logdet1);
end
